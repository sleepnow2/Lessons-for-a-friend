...
function mul_by_two(number) {
    return number*2;
}
console.log("mul_by_two(5) =",mul_by_two(5));

function mul_two_numbers(num1, num2) {
    return num1*num2;
}
console.log("mul_two_numbers(6, 3) =",mul_two_numbers(6, 3));

function calc_factorial(inp) {
    let result = 1;
    while (inp > 0) {
        result *= inp;
        inp -= 1;
...
console.log("calc_factorial(10) =",calc_factorial(10));
...
function find_y_from_two_points(x, x1, y1, x2, y2) {
    function find_slope(x1, y1, x2, y2) {
        return (y1-y2)/(x1-x2);
    }
    function find_y_intercept(x,y,m){
        return -x*m + y;
    }
...
console.log("find_y_from_two_points(3,1,2,2,4) =",find_y_from_two_points(3,1,2,2,4))
...
function find_y_from_two_points_v2(x, x1, y1, x2, y2){
    function find_slope(){
        return (y1-y2)/(x1-x2);
    }
    function find_y_intercept(m){
        return -x1*m + y1;
    }
    slope = find_slope();
...
console.log("find_y_from_two_points_v2(3,1,2,2,4) =",find_y_from_two_points_v2(3,1,2,2,4))

// you can even pass functions as inputs to other functions...
function apply(f, val) {
    return f(val)
}
console.log("apply(mul_by_two, 4) =", apply(mul_by_two, 4))

// or get functions out of functions...
function gen_linear(slope, intercept) {
    function result(x) {
...
function gen_linear_lambda(slope, intercept) {
    return x => slope*x + intercept;
}
line_lambda = gen_linear_lambda(2, 2)

console.log("line_lambda(2) =", line_lambda(2))
